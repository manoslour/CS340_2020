...
term:		L_PAR 						{printf("term: L_PAR at line %d --> %s\n", yylineno, yytext);}
			...
			|INCR	{
						int result = generalLookUp(yytext,currscope);
						if (result < 3) {addError("Error trying to incriment a function",yytext,yylineno);}
					} 
			lvalue				{printf("term: INCR lvalue at line %d --> %s\n", yylineno, yytext);}
